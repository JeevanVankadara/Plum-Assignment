...
import type { BackendClaimDto } from "../lib/types";

interface DecisionMixItem {
  status: string;
  count: number;
}
...
export default function Analytics() {
  const [claims, setClaims] = useState<BackendClaimDto[]>([]);
                ...
                return (
                  <div key={item.status}>
                    ...
                    <div className="h-2 rounded-full bg-background overflow-hidden">
                      <div className={`h-full ${metricColor(item.status)}`} style={{ width: `${width}%` }} />
                    </div>
                  </div>
                );
              })}
